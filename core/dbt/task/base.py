...
from dbt.contracts.results import (
    RunModelResult, collect_timing_info, RunStatus
)
...
class BaseRunner(metaclass=ABCMeta):
    ...
    def get_result_status(self, result) -> Dict[str, str]:
        ...
        # TODO(kw) do we need to handle this case?
        # elif result.fail:
        #     return {'node_status': 'failed'}
        # TODO(kw) do we need to handle this case as well?
        # elif result.warn:
        #     return {'node_status': 'warn'}
        elif result.status is RunStatus.Success:
            return {'node_status': 'passed'}
        else:
            raise RuntimeError(f"unhandles result {result}")
    ...
    def safe_run(self, manifest):
        ...
        try:
            ...
            if (
                ...
                result.status is not RunStatus.Error and error is None
            ):
                error = exc_str
