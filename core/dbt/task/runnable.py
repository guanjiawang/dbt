...
class GraphRunnableTask(ManifestTask):
    ...
    def call_runner(self, runner):
        ...
        if result.status is RunStatus.Error and fail_fast:
            self._raise_next_tick = FailFastException(
                message='Failing early due to test failure or runtime error',
                result=result,
                node=getattr(result, 'node', None)
            )
        elif result.status is RunStatus.Error and self.raise_on_first_error():
    ...
    def interpret_results(self, results):
        ...
        # or r.fail] <- TODO(kw) handle fail?
        failures = [r for r in results if r.status is RunStatus.Error]
